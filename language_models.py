...
class LocalvLLM(LanguageModel):
    ...
    def batched_generate(self, convs_list: list[list[dict]],
                         ...
                         top_p: float,
                         extra_eos_tokens: list[str] = None) -> list[str]:
        # 为了兼容 PAIR 的接口，把 messages 转换为 prompt
        prompts = []
        for messages in convs_list:
            conv = self._init_conv_template()
            for msg in messages:
        ...
        self.sampling_params = vllm.SamplingParams(
            ...
            top_p=top_p,
            max_tokens=max_n_tokens,
            stop=extra_eos_tokens or [],
        )

        responses = self._get_responses(prompts, max_new_tokens=max_n_tokens)
